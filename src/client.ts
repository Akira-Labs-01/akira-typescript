...
export interface ClientOptions {
   ...
   *
   * Defaults to process.env['AKIRA_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['AKIRA_LOG'] or 'warn' if it isn't set.
   */
...
/**
 * API Client for interfacing with the Akira API.
 */
export class Akira {
  apiKey: string;
   ...
   * API Client for interfacing with the Akira API.
   *
   * @param {string | undefined} [opts.apiKey=process.env['AKIRA_API_KEY'] ?? undefined]
   * @param {string} [opts.baseURL=process.env['AKIRA_BASE_URL'] ?? https://api.example.com] - Override the default base URL for the API.
   ...
   */
  constructor({
    baseURL = readEnv('AKIRA_BASE_URL'),
    apiKey = readEnv('AKIRA_API_KEY'),
    ...opts
  }: ClientOptions = {}) {
    if (apiKey === undefined) {
      throw new Errors.AkiraError(
        "The AKIRA_API_KEY environment variable is missing or empty; either provide it, or instantiate the Akira client with an apiKey option, like new Akira({ apiKey: 'My API Key' }).",
      );
    }
    ...
    this.timeout = options.timeout ?? Akira.DEFAULT_TIMEOUT /* 1 minute */;
    ...
    this.logLevel =
      ...
      parseLogLevel(readEnv('AKIRA_LOG'), "process.env['AKIRA_LOG']", this) ??
      defaultLogLevel;
  ...
  static AkiraError = Errors.AkiraError;
...
Akira.Sandboxes = Sandboxes;
Akira.Snapshots = Snapshots;

export declare namespace Akira {
